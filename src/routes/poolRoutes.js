...
poolRouter.get('/users', (req, res) => {
  try {
    ...
    connection.query(findUserQuery, (findUserError, findUserResults) => {
      ...
      if (findUserError || findUserResults.length !== 1)     res.status(403).end()// .json message: 'Error finding user'})
      ...
      if (db.extra1) {
        ...
        if (hashedPassword.fullHashedPassword !== db.extra1) res.status(403).end() // .json message: 'Invalid credentials'})
        jwt.sign({ id: db.id, username: db.username }, secretKey, { expiresIn: '1 day'}, (err, token) => {
          if (err) res.status(500).end()// .json message: 'Error signing data'})
        ...
        hashPasswordLegacy(password) // hash password using old legacy password scheme
          .then((legacyPassword) => {
            ...
            if (db.password !== legacyPassword) res.status(403).end() //.json({ message: 'Invalid credentials'})
            ...
            connection.query(insertNewPasswordQuery, (err, insertNewPasswordResults) => {
              if (insertNewPasswordResults.affectedRows !== 1) res.status(500).end() // .json({ message: 'Error inserting new password'})
              jwt.sign({ id: db.id, username: db.username }, secretKey, { expiresIn: '1 day'}, (err, token) => {
                if (err) res.status(500).end()// .json message: 'Problem signing data'})
                console.log('signed token is: ', token, ' and error is: ', err)
                res.status(200).json({ token, id: db.id })
              })                
            })
    ...
    res.status(500).end()// .json message: e})
  }
})
